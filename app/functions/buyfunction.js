...
const fee = 10000; // Uniswap pool fee bps 500, 3000, 10000
const buyAmount = ethers.parseUnits('0.001', 'ether'); //amount buy with in eth
const targetPrice = BigInt(35); // target exchange rate
const targetAmountOut = buyAmount * targetPrice;

const sellAmount = ethers.parseUnits('0.001', 'ether'); // Assuming 18 decimals for the token
...
const buyTokens = async () => {
  ...
  await tx.wait();
  console.log("Token Purchase completed: ",tx.hash);
}
